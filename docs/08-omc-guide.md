...
oh-my-claudecode(OMC)는 Claude Code 위에 **멀티 에이전트 오케스트레이션**을 얹어주는 오픈소스 플러그인이에요. Claude Code 하나로도 충분하지만, OMC를 쓰면 여러 전문 에이전트가 역할을 나눠서 분석·구현·검증을 동시에 처리합니다. 혼자 코딩하다가 팀을 얻는 느낌이에요.

이 가이드에서는 OMC의 주요 기능과 실전에서 쓰는 법을 다룹니다.
...
OMC 활용법을 익혔다면 [32. Superpowers 활용 가이드](./32-superpowers-guide.md)에서 TDD·brainstorming 같은 개발 방법론 프레임워크를 알아보세요.

자동화에 관심이 있다면 [09. 자동화의 힘](./09-automation.md)으로 바로 넘어가도 됩니다.
