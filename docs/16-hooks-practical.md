...
!!! warning "흔한 함정 2가지"
    **exit 코드 혼동**: `exit 1`은 에러 로그만 남기고, `exit 2`만 도구 실행을 차단합니다. 헷갈리면 Claude가 의도치 않게 작업을 계속하거나 멈춥니다.
    **무한 루프**: PostToolUse 훅에서 파일을 수정하면 → 다시 훅이 실행 → 또 수정... 이 루프에 빠질 수 있어요. 반드시 **조건 체크**로 불필요한 재실행을 막으세요.
