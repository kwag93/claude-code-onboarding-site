# 02. AI 마인드셋

AI를 효과적으로 활용하기 위해 알아야 할 관점입니다.

---

## AI는 단순한 지시 수행자가 아니다

많은 사람들이 AI를 "명령하면 실행하는 도구"로 생각합니다.
하지만 이 관점은 AI의 잠재력을 낭비하게 만듭니다.

### 도서관 사서를 떠올려보세요

AI는 **광대한 지식 베이스를 가진 도서관 사서**와 같습니다.

**잘못된 접근** ❌

```
"자바스크립트로 로그인 기능 만들어줘"
```

→ 사서는 자기가 생각하기에 맞는 책을 그냥 줄 뿐입니다.
→ 당신이 원하는 것과 다를 수 있습니다.

**올바른 접근** ⭕

```
"JWT 기반 인증을 구현하려고 하는데,
세션 방식과 비교했을 때 장단점이 뭐야?
우리 프로젝트에서는 어떤 게 더 적합할까?"
```

→ 사서는 관련된 다양한 자료들을 제시합니다.
→ 당신은 그 중에서 필요한 것을 **취사선택**합니다.

### 핵심 전환

| Before        | After           |
| ------------- | --------------- |
| "해줘"        | "질문한다"      |
| 결과를 받는다 | 선택지를 받는다 |
| AI가 결정     | 내가 결정       |

---

## 인지 오프로딩의 위험성

Anthropic의 연구에 따르면, AI에 과도하게 의존하면 **학습과 스킬 형성이 저해**됩니다.

### 연구 결과 요약

52명의 전문 개발자를 대상으로 한 실험에서:

- AI를 사용한 그룹이 이해도 테스트에서 **17% 낮은 점수**
- 특히 **코드 읽기, 디버깅 능력**이 저하됨
- 완전히 위임한 그룹은 작업은 빨랐지만, 학습은 거의 없었음

### 6가지 AI 상호작용 패턴

**낮은 학습 효과 (점수 40% 미만)**

| 패턴               | 설명                                |
| ------------------ | ----------------------------------- |
| AI 위임            | 코드 작성을 전적으로 AI에게 맡김    |
| 점진적 의존        | 처음엔 질문하다가 결국 모든 걸 맡김 |
| 반복적 디버깅 의존 | AI에게 계속 디버깅 요청             |

**높은 학습 효과 (점수 65% 이상)**

| 패턴             | 설명                    |
| ---------------- | ----------------------- |
| 설명 요청        | "왜 이렇게 했어?" 질문  |
| 개념 질문        | 기술적 개념에 대해 질문 |
| 독립적 사고 유지 | AI 답변을 검토하고 판단 |

### 실천 방법

```
# 나쁜 예
"이 버그 고쳐줘" → [복붙] → 다음 버그

# 좋은 예
"이 버그 고쳐줘"
→ "왜 이런 버그가 발생했어?"
→ "비슷한 패턴의 버그를 미리 방지하려면?"
→ [이해하고 적용]
```

---

## 넓은 팔을 가진 도서관 활용하기

AI를 "일 시키는 도구"가 아닌 "탐색하는 파트너"로 생각하세요.

### 추천 활용 방식

**1. 선택지 요청하기**

```
"이 문제를 해결하는 방법을 3가지 제안해줘.
각각의 장단점도 함께."
```

**2. 사서의 추천 받기**

```
"지금 이 방식으로 구현했는데,
비슷한 상황에서 자주 쓰이는 다른 패턴 있어?"
```

**3. 취사선택하기**

```
"두 번째 방법이 우리 상황에 맞는 것 같아.
이걸로 구체화해보자."
```

### 기억할 것

> **AI가 제공하는 생산성 향상은 곧 역량 향상을 의미하지 않습니다.**
>
> — Anthropic Research, 2025

AI는 당신의 판단력을 대체하는 것이 아니라,
**더 나은 판단을 위한 자료를 제공**하는 것입니다.

---

## 실천 체크리스트

- [ ] AI에게 "해줘"보다 "어떻게 생각해?"로 시작하기
- [ ] 결과를 받으면 "왜 이렇게 했어?" 질문하기
- [ ] 여러 선택지를 요청하고 내가 선택하기
- [ ] 복붙하기 전에 코드를 이해했는지 확인하기
- [ ] 모르는 개념이 나오면 설명 요청하기

---

## 다음 단계

AI를 대하는 관점이 정리되었다면 [03. 실무 워크플로우](./03-workflows.md)에서 실제 프로젝트에 적용하는 방법을 알아보세요.

---

**참고 자료**:

- [How AI assistance impacts the formation of coding skills](https://www.anthropic.com/research/AI-assistance-coding-skills) - Anthropic Research
- [Anthropic Study Finds AI Coding Assistants Speed Up Work But Reduce Skills](https://www.sovereignmagazine.com/education/anthropic-study-finds-ai-coding-assistants-speed-up-work-but-reduce-skills/)
