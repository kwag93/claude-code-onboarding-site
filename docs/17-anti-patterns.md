...
그다음 `/clear` 후 `@HANDOFF.md 읽고 작업 이어가줘`로 새 세션을 시작합니다.
...
**원인**: 로그인 기능을 만들다가 "아 참, 대시보드 UI도 수정해줘"라고 하면, 두 주제의 컨텍스트가 섞여서 **컨텍스트 오염**이 발생합니다.
...
**증상**: 언제 `/clear`를 해야 하는지 몰라서 품질이 떨어진 대화를 계속 끌고 간다. 또는 반대로, 필요한 맥락까지 날려버린다.

**원인**: `/clear`와 `/compact`의 차이를 모르고, 적절한 타이밍을 판단하지 못합니다.

| 명령 | 동작 | 사용 시점 |
|------|------|-----------|
| `/clear` | 대화 전체 초기화 | 주제 전환, 새 작업 시작 |
| `/compact` | 기존 맥락을 요약 압축 | 같은 작업 중 토큰 절약 |

**`/clear` 해야 할 때**:
- 이전 주제와 완전히 다른 새 작업을 시작할 때
- AI 답변 품질이 눈에 띄게 떨어질 때
- 논리적 단위의 작업이 완료되었을 때

**`/clear` 하면 안 될 때**:
- 현재 작업이 진행 중일 때 (맥락 손실!)
- 아직 참조해야 할 대화 내용이 있을 때

> 💡 **Status Line 활용 팁**: [Status Line](25-statusline-guide.md)에서 컨텍스트 사용량을 실시간으로 확인할 수 있어요. 게이지가 80%를 넘어가면 `/clear` 또는 `/compact`를 고려하세요.

---

## 실수 11: HANDOFF.md를 활용하지 않는다

**증상**: 긴 대화 후 `/clear`를 하면 모든 맥락이 사라진다. 다음 날 이어서 하려면 처음부터 다시 설명해야 한다.

**원인**: 세션 간 컨텍스트 인계 방법을 모릅니다.
...
**언제 HANDOFF.md를 만들어야 하나?**:
- 대화가 길어져서 `/clear`가 필요할 때
- 퇴근 전, 내일 이어서 할 작업이 있을 때
- 컨텍스트 오버플로우가 임박할 때
- 다른 사람(또는 다른 세션)에게 작업을 넘길 때
