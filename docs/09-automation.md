...
5분마다 `/commit`을 실행합니다. 기본 간격은 10분이에요.

### 활용 예시

```text
# 5분마다 빌드 상태 확인
/loop 5m "빌드 상태 확인하고 실패하면 알려줘"

# 10분마다 PR 모니터링
/loop 10m "열린 PR 목록 확인하고 리뷰 필요한 것 알려줘"
...
Claude Code에 **cron 스케줄링**이 내장되어 있어요. 정해진 시간에 자동으로 작업을 실행합니다.
...
> Cron은 Claude Code가 실행 중이지 않아도 백그라운드에서 동작해요. 자세한 설정은 [공식 문서](https://code.claude.com/docs/en/scheduled-tasks)를 참고하세요.
