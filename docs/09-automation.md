...
자연어로 한 번만 실행할 작업을 예약해요. 실행 후 자동으로 삭제됩니다.
...
Cron 작업은 현재 실행 중인 Claude Code 프로세스에 귀속되어요. 종료하면 작업도 사라집니다.

- 반복 작업: 생성 후 **3일 만료**
- 세션당 최대 **50개** 작업
- Claude가 응답 중일 때는 실행 안 됨 — 유휴 상태일 때 실행
- 모든 시간은 **로컬 타임존** 기준

### Jitter (실행 시간 분산)

동시 실행을 막기 위해 의도적으로 약간의 오차를 줘요.

- **반복 작업**: 기간의 최대 10% 늦게 실행 (최대 15분)
- **일회성 작업**: 최대 90초 일찍 실행
- 작업 ID 기반으로 결정론적으로 계산돼요
...
tmux 또는 screen 안에서 Claude Code를 실행하면 터미널을 닫아도 세션이 유지돼요. 반복 cron 작업을 안정적으로 운영하고 싶을 때 유용합니다.
