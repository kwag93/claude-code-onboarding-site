...
당신의 워크플로우는 유일해요. 범용 도구로 충분하지 않다면, 직접 만들면 됩니다.
...
가장 간단한 확장부터 시작해요. 스킬 하나를 만드는 데 5분이면 충분합니다.
...
트리거 키워드를 포함한 요청을 하면 스킬이 자동으로 활성화돼요. `/korean-commit`처럼 슬래시 명령으로도 호출할 수 있습니다.
...
트리거 키워드를 포함한 요청을 하면 스킬이 자동으로 활성화돼요.

### 5단계: 반복 개선

사용하면서 부족한 점을 발견하면 스킬 파일을 수정하세요. 스킬은 마크다운 파일이니까 언제든 편집할 수 있어요.

---

## 첫 번째 플러그인 만들기

스킬 하나로는 부족할 때, 여러 스킬 + Hook + 에이전트를 묶은 **플러그인**을 만들 수 있어요.
...
외부 시스템의 이벤트를 Claude Code에 전달하는 채널도 만들 수 있어요. [Channels 가이드](28-channels.md)에서 개념을 먼저 확인하세요.
...
사내 API나 자체 서비스를 Claude Code에 연결할 수 있어요. MCP 서버를 만들면 됩니다.
